...
| 样式覆盖 | 模板之上叠加文档级样式：主题色/字号/行高/段距/字距、标题/引用/表格/分割线微调、提示盒/步骤条等样式盒子、样式刷子跨段落复制；随文档保存，导出 MD 携带、导入恢复 |
| 局部卡片 | 编辑器内置 24 张主题联动卡片，按重点提示 / 金句摘录 / 结论复盘 / 步骤流程 / 清单记录 / 标题正文六类场景筛选，可选区套用、直接插入、换卡和无损移除 |
| 代码高亮 | 17 种高亮方案（下拉精选 6 种常用），随文章主题自动联动 |
