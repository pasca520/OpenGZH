(function openGzhContentScript(root) {
  ...
  async function readImageData(reference, options = {}) {
    ...
    try {
      ...
      if (ref.startsWith('data:image/')) return typeof image.dataUrl === 'string' ? image.dataUrl : ref;
  ...
  function createImageResponder({ port, read = readImageData, readImageData: readOverride } = {}) {
    ...
    async function processNext() {
      ...
      try {
        const dataUrl = await reader(message.ref);
        post({
          type: 'IMAGE_DATA',
  ...
  function createUi({
    ...
    for (const platformId of PLATFORM_IDS) {
      ...
      checkbox.addEventListener('change', () => {
        ...
        try {
          if (state.selected.length) storage?.setItem?.(STORAGE_KEY, JSON.stringify(state.selected));
          else storage?.removeItem?.(STORAGE_KEY);
        } catch {
          // Selection still works for this page lifetime when storage is unavailable.
        }
      });
      ...
      retry.addEventListener('click', () => {
        ...
        post({
          ...
          ...(state.taskId ? { taskId: state.taskId, platformId } : { platformIds: state.selected.slice() }),
        });
      });
    }
    ...
    function setLocked(locked) {
      ...
      for (const { checkbox } of rowMap.values()) checkbox.disabled = locked;
    }
    ...
    function setStatus(platformId, status, message) {
      ...
      if (!row) return;
      row.status.textContent = STATUS_LABELS[status] || STATUS_LABELS.unknown;
      if (status === 'success' && typeof message?.draftUrl === 'string' && message.draftUrl) {
        row.draft.href = message.draftUrl;
        row.draft.hidden = false;
      }
      if (status === 'failed' && typeof message?.error === 'string') row.status.textContent = message.error;
    }
    ...
    function sendCheckAuth() {
      for (const platformId of PLATFORM_IDS) setStatus(platformId, 'checking-auth');
    ...
    async function startBatch() {
      ...
      try {
        ...
        post({ type: 'START_BATCH', taskId: state.taskId, platformIds: state.selected.slice(), snapshot: article });
      } catch (error) {
    ...
    function onMessage(message) {
      ...
      if (message?.type === 'BATCH_COMPLETE') {
        if (state.taskId && message.taskId === state.taskId) finishTask();
        return;
      }
