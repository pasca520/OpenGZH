...
export function createStoredZip(files) {
  ...
  const entries = files.map((file) => {
    ...
    return { name: String(file.name), nameBytes, data: file.data, crc: crc32(file.data) };
  });
  ...
  for (const entry of entries) {
    ...
    parts.push(buildLocalHeader(entry), entry.nameBytes, entry.data);
  ...
  for (const { entry, offset: localOffset } of central) {
    centralParts.push(buildCentralRecord(entry, localOffset), entry.nameBytes);
